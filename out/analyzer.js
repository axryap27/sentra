...
class SecurityAnalyzer {
    ...
    async scanFile(uri) {
        ...
        try {
            ...
            const incrementalEnabled = config.get('enableIncrementalScanning', true);
        ...
        await vscode.window.withProgress({
            ...
            for (const file of supportedFiles) {
                ...
                try {
                    ...
                    const content = document.getText();
                    // Check if incremental scanning is enabled and file has changed
                    const incrementalEnabled = vscode.workspace.getConfiguration('secureCodeAnalyzer').get('enableIncrementalScanning', true);
    ...
    async findSupportedFiles() {
        ...
        const maxFileSize = config.get('maxFileSize', 1024 * 1024); // 1MB default
        const filteredFiles = [];
