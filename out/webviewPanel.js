...
exports.SecurityReportPanel = void 0;
const vscode = require("vscode");
class SecurityReportPanel {
    constructor(context) {
    ...
    showReport(reports) {
        if (this.panel) {
            ...
            this.panel = vscode.window.createWebviewPanel('securityReport', 'Security Analysis Report', vscode.ViewColumn.Two, {
                enableScripts: true,
    ...
    getWebviewContent(reports) {
        const totalIssues = reports.reduce((sum, report) => sum + report.issues.length, 0);
        const highSeverityCount = reports.reduce((sum, report) => sum + report.issues.filter(issue => issue.severity === 'High').length, 0);
        const mediumSeverityCount = reports.reduce((sum, report) => sum + report.issues.filter(issue => issue.severity === 'Medium').length, 0);
        const lowSeverityCount = reports.reduce((sum, report) => sum + report.issues.filter(issue => issue.severity === 'Low').length, 0);
        return `
<!DOCTYPE html>
<html lang="en">
<head>
...
exports.SecurityReportPanel = SecurityReportPanel;
